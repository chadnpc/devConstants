...
#!/usr/bin/env pwsh
# .SYNOPSIS
#   devConstants testScript v0.1.1
# .EXAMPLE
#   ./Test-Module.ps1 -version 0.1.1
#   Will test the module in ./BuildOutput/devConstants/0.1.1/
# .EXAMPLE
#   ./Test-Module.ps1
#   Will test the latest  module version in ./BuildOutput/devConstants/
param (
  ...
  [ValidateScript({
      ...
      $b_Path = [IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'devConstants')
...
begin {
  $TestResults = $null
  $BuildOutput = [IO.DirectoryInfo]::New([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'devConstants'))
  ...
  $BuildOutDir = Resolve-Path $([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'devConstants', $version)) -ErrorAction Ignore | Get-Item -ErrorAction Ignore
  if (!$BuildOutDir.Exists) { throw [System.IO.DirectoryNotFoundException]::new($BuildOutDir) }
  $manifestFile = [IO.FileInfo]::New([IO.Path]::Combine($BuildOutDir.FullName, "devConstants.psd1"))
  ...
  if (!(Test-Path -Path $([IO.Path]::Combine($PSScriptRoot, "devConstants.psd1")) -PathType Leaf -ErrorAction Ignore)) { throw [System.IO.FileNotFoundException]::New("Module manifest file Was not Found in '$($BuildOutDir.FullName)'.") }
  $script:fnNames = [System.Collections.Generic.List[string]]::New(); $testFiles = [System.Collections.Generic.List[IO.FileInfo]]::New()
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'devConstants.Integration.Tests.ps1')))
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'devConstants.Features.Tests.ps1')))
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'devConstants.Module.Tests.ps1')))
}
